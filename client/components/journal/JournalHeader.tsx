...
export function JournalHeader({ journalId, title, description, users, onEntryCreated }: JournalHeaderProps) {
    return (
        <div className="mx-auto w-full max-w-7xl px-4 pt-8">
            <div className="relative overflow-hidden rounded-3xl border border-border p-8 shadow-lg">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex items-center -space-x-2">
                                {users.slice(0, 5).map((user) => (
                                    <Avatar key={user.id} className="h-10 w-10 border-2 border-white/20">
                    ...
                    <div className="flex items-center gap-3">
                        <AddEntryDialog journalId={journalId} onEntryCreated={onEntryCreated}>
                            <Button className="rounded-xl px-6 py-6 h-auto flex items-center gap-2 font-semibold shadow-md transition-all active:scale-95 dark:bg-[#FF7A54] dark:hover:bg-[#ff693f] dark:text-black">
                                <Camera className="w-5 h-5" />
                                Upload Today's Journal
                            </Button>
                        </AddEntryDialog>
